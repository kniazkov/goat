...
namespace g0at
{
    namespace parser
    {
        class statement_throw : public pattern
        {
        public:
            statement_throw(parser_data *_data)
                : pattern(&_data->throw_keywords, _data)
            {
            }
        ...
        protected:
            ...
            {
                ast::keyword_throw *kw = tok->to_keyword_throw();
                assert(kw != nullptr);

                if (!kw->next)
                {
                    lib::pointer<ast::token> stmt = new ast::statement_throw(kw, nullptr);
                    kw->replace(stmt);
                    return 0;
                }
        ...
        lib::pointer<pattern> grammar_factory::create_pattern_statement_throw()
        {
            return new statement_throw(data);
        }
    };
};
